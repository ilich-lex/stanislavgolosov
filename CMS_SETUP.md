...
Публичный endpoint `/auth/v1/settings` не показывает список разрешённых Redirect URLs, поэтому пункты 2–3 нужно проверить вручную в `Authentication → URL Configuration` перед публикацией восстановления пароля. Для отдельного локального теста письма временно добавьте точный preview-адрес, например `http://127.0.0.1:4177/admin/**`, и удалите его после проверки.

Восстановление работает штатным способом Supabase Auth:

1. На `/admin/` пользователь нажимает «Забыли пароль?» и вводит e-mail.
2. `resetPasswordForEmail()` отправляет письмо без раскрытия того, зарегистрирован адрес или нет.
3. Ссылка возвращает пользователя на `https://golosovsv.ru/admin/?mode=recovery`.
4. Админка принимает только настоящее событие `PASSWORD_RECOVERY`, проверяет UUID по `admin_users` и вызывает `updateUser()`.
5. После успешной смены recovery-сессия завершается, пользователь входит с новым паролем.

Для production-писем рекомендуется настроить собственный SMTP в `Authentication → SMTP Settings`: встроенная тестовая отправка Supabase имеет строгие лимиты и не предназначена для гарантированной коммерческой доставки.
...
После базовой миграции примените `supabase/002_price_list.sql` отдельным запросом. Она создаёт таблицу `price_items`, RLS-policies и переносит семь исходных позиций прайса с сохранением текста, цен, порядка и акцентного оформления банкротства.
...
10. В разделе «Прайс-лист» исходные семь позиций совпадают с HTML fallback.
11. Изменение цены сразу появляется на публичном сайте; после проверки исходная цена восстановлена.
12. Тестовая позиция проходит цикл добавление → редактирование → удаление и не остаётся в базе.
13. При недоступном API публичная страница продолжает показывать исходный HTML-прайс.
...
- `supabase/001_cms_schema.sql`, `supabase/002_price_list.sql` и документацию — для воспроизводимости настроек.
...
После этого изменения дел, фотографии и прайс-листа выполняются через Supabase и больше не требуют commit/push.
